...
Generations show a temporary progress message: **Working.**, **Rendering.**, or **Head to head · 1/2**. Attachment and conversation loading report their own stages. On completion, controls move to the last answer or file message and the progress message is removed, where Discord permits these edits. A divider separates the answer from its muted model/time footer, including embed replies.

Text answers offer **Go deeper**, **Shorten**, and **Challenge this** to build on the answer with its prepared conversation. **Change model** opens a private, paginated dropdown to rerun the original prompt with another configured model, without changing the channel default. Follow-ups preserve response privacy and use the same permissions, cooldown, concurrency, timeout, and stop/retry handling as slash commands. Repeated follow-ups obey the configured history and text limits. Comparisons also offer these actions; changing their model replaces the first contender.

Battles show anonymous A/B labels in progress, footers, and downloaded answers until voting; generated answer text itself may contain model self-identification. A single requester vote reveals the mapping in the same public/private scope as the battle. Votes are session-only, with no leaderboard. Both contenders get identical input and neither sees the other's answer. In debates, the second model sees the first model's argument. Each submitted steering direction runs exactly one more two-model round; nothing continues automatically. Retrying repeats the selected battle or debate round with its original input. An incomplete battle cannot be voted on; an incomplete debate must be retried before continuing.

Only the original requester can use controls, and current bot permissions are checked. **Download** sends a private UTF-8 `answer.md` file containing the text response. Controls expire after 15 minutes or a bot restart; expiry disables them without replacing the answer. `/stop` and `/retry` remain available as slash commands. Set `response_buttons: false` to disable ordinary answer controls and progress; model/time footers remain. Battles and debates always show the controls needed to vote or continue.
