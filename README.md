...
Generations show a temporary progress message with a single **Download** button. Progress reports stages such as reading attachments, reading the conversation, generating an image, or comparing model 1 of 2. On completion, the button moves to the last answer or file message and the progress message is removed, where Discord permits these edits. The answer footer shows the model and elapsed time. Only the original requester may use the button, and current bot permissions are checked. `/stop` and `/retry` remain available as slash commands. Download sends a private UTF-8 `answer.md` file containing the text response and becomes available after generation finishes, where text is available. The button expires after 15 minutes or a bot restart; expiry disables it without replacing the answer. Set `response_buttons: false` to disable the button and progress messages; model/time footers remain.
